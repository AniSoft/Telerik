...
namespace GeometryAPI
{
    ...
    public class Circle : Figure, IFlat, IAreaMeasurable
    {
        ...
        public Circle(Vector3D circleCenter, double radius)
            : base(circleCenter)
        {
            this.Radius = radius;
        }
        ...
        public Vector3D GetNormal()
        {
            Vector3D B = new Vector3D(CircleCenter.X + Radius, CircleCenter.Y, CircleCenter.Z);
            Vector3D C = new Vector3D(CircleCenter.X, CircleCenter.Y + Radius, CircleCenter.Z);
            Vector3D normal = Vector3D.CrossProduct(B - CircleCenter, C - CircleCenter);
            normal.Normalize();
            return normal;
        }
    ...
    public class Cylinder : Figure, IAreaMeasurable, IVolumeMeasurable
    {
        ...
        public double GetArea()
        {
            ...
            double sideArea = 2 * Math.PI * this.Radius * this.GetHeight();
        ...
        {
            Vector3D H = this.TopCenter - this.BottomCenter;
            return H.Magnitude;
        }
    }

    public class AddvancedFigureControler : FigureController
    {
        public override void ExecuteFigureCreationCommand(string[] splitFigString)
        {
            switch (splitFigString[0])
            {
                ...
                case "cylinder":
                    {
                        Vector3D bottomCenter = Vector3D.Parse(splitFigString[1]);
                        Vector3D topCenter = Vector3D.Parse(splitFigString[2]);
                        double radius = double.Parse(splitFigString[3]);
                        currentFigure = new Cylinder(bottomCenter, topCenter, radius);
                        break;
                    }
            }
        ...
        protected override void ExecuteFigureInstanceCommand(string[] splitCommand)
        {
            switch (splitCommand[0])
            {
                case "area":
                    {
                        ...
                        {
                            Console.WriteLine("{0:F2}",areaMeasurableFigure.GetArea());
                        }
                        else
                        {
                ...
                case "volume":
                    {
                        ...
                        {
                            Console.WriteLine("{0:F2}",volumeMeasurableFigure.GetVolume());
                        }
                        else
                        {
                            Console.WriteLine("undefined");
                ...
                case "normal":
                    {
                        IFlat hasNormalFigure = currentFigure as IFlat;

                        if (hasNormalFigure != null)
                        {
                            Console.WriteLine(hasNormalFigure.GetNormal());
                        }
                        else
                        {
                            Console.WriteLine("undefined");
...
namespace AcademyGeometry
{
    class Program
    {
        ...
        {
            return new AddvancedFigureControler();
        }
